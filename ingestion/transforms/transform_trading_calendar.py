...
def run():
    with StepTimer() as timer:
        ...
        log_info(logger, "Transform started", step="transform",
                 target="bronze.trading_calendar", range_start=start, range_end=end)
        ...
        for yf_code in used_exchanges:
            ...
            if not xc_code:
                log_warning(logger, "Cannot resolve exchange",
            ...
            total += count
            log_info(logger, "Exchange processed", step="transform",
                     exchange=yf_code, xc_code=xc_code, rows=count)

        cursor.close()
        conn.close()

    log_info(logger, "Transform complete", step="transform",
             target="bronze.trading_calendar", records_inserted=total,
             exchanges=len(used_exchanges), duration_ms=timer.duration_ms)
